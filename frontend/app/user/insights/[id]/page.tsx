...
const STATUS_DISPLAY: Record<string, { label: string; style: string }> = {
  unread: { label: "New", style: "bg-sky-100 text-sky-700" },
  read: { label: "Read", style: "bg-[#eef6f6] text-[#6f8c91]" },
  approved: { label: "Approved ✓", style: "bg-emerald-100 text-emerald-700" },
  dismissed: { label: "Dismissed", style: "bg-[#eef6f6] text-[#6f8c91]" },
};
...
export default function InsightDetailPage() {
  ...
  if (loading) {
    return (
      <div className="px-4 py-6 sm:mx-auto sm:max-w-md sm:px-0">
        <div className="mb-6 h-4 w-24 animate-pulse rounded bg-[#eef6f6]" />
        <div className="space-y-3">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-20 animate-pulse rounded-2xl bg-[#eef6f6]" />
          ))}
        </div>
      </div>
    );
  ...
  if (error || !insight) {
    return (
      <div className="px-4 py-6 sm:mx-auto sm:max-w-md sm:px-0">
        <Link href="/user" className="mb-4 inline-flex items-center gap-1 text-sm text-[#6f8c91] hover:text-[#10363b]">
          ← Back
        </Link>
        <p className="text-sm text-[#6f8c91]">Insight not found.</p>
      </div>
    );
  }
  ...
  return (
    <div className="pb-2">
      <div className="px-4 py-6 sm:mx-auto sm:max-w-md sm:px-0">
        ...
        <Link
          href="/user"
          className="mb-4 inline-flex items-center gap-1 text-sm text-[#6f8c91] hover:text-[#10363b]"
        >
          ← Back to My Home
        </Link>
        ...
        <div className="mb-5">
          <div className="mb-2 flex items-start gap-3">
            ...
            <div className="flex-1">
              <h1 className="text-lg font-bold leading-snug text-[#10363b]">
          ...
          <p className="pl-12 text-xs text-[#6f8c91]">
        ...
        <section className="mb-4">
          <div className="rounded-2xl border border-[rgba(157,207,212,0.40)] bg-gradient-to-b from-[rgba(255,255,255,0.94)] to-[rgba(243,249,249,0.88)] p-4 shadow-[0_4px_20px_rgba(0,74,82,0.07)]">
        ...
        <section className="mb-4">
          <h2 className="mb-3 text-xs font-semibold uppercase tracking-wider text-[#86CCD2]">
            What triggered this insight
          </h2>
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-2xl border border-[rgba(157,207,212,0.40)] bg-gradient-to-b from-[rgba(255,255,255,0.94)] to-[rgba(243,249,249,0.88)] px-4 py-3 shadow-[0_4px_16px_rgba(0,74,82,0.06)]">
              <p className="text-[10px] font-medium text-[#6f8c91]">This Week</p>
              <p className="mt-0.5 text-xl font-bold text-[#10363b]">
                {insight.this_week_kwh.toFixed(1)}
                <span className="ml-1 text-sm font-normal text-[#6f8c91]">kWh</span>
              </p>
            </div>
            <div className="rounded-2xl border border-[rgba(157,207,212,0.40)] bg-gradient-to-b from-[rgba(255,255,255,0.94)] to-[rgba(243,249,249,0.88)] px-4 py-3 shadow-[0_4px_16px_rgba(0,74,82,0.06)]">
              <p className="text-[10px] font-medium text-[#6f8c91]">Last Week</p>
              <p className="mt-0.5 text-xl font-bold text-[#10363b]">
                {insight.last_week_kwh.toFixed(1)}
                <span className="ml-1 text-sm font-normal text-[#6f8c91]">kWh</span>
              </p>
            </div>
            <div className={`rounded-2xl border px-4 py-3 shadow-[0_4px_16px_rgba(0,74,82,0.06)] ${
              vsLastWeekPositive
              ...
              <p className={`mt-0.5 text-xl font-bold ${
                vsLastWeekPositive ? "text-red-600" : "text-emerald-600"
              }`}>
                {vsLastWeekPositive ? "+" : ""}{insight.change_pct.toFixed(1)}%
              </p>
            </div>
            <div className="rounded-2xl border border-[rgba(157,207,212,0.40)] bg-gradient-to-b from-[rgba(255,255,255,0.94)] to-[rgba(243,249,249,0.88)] px-4 py-3 shadow-[0_4px_16px_rgba(0,74,82,0.06)]">
              <p className="text-[10px] font-medium text-[#6f8c91]">Est. Cost</p>
              <p className="mt-0.5 text-xl font-bold text-[#10363b]">
          ...
          <div className="mt-3 space-y-2">
            ...
            <div className="flex items-center gap-3 rounded-xl border border-[rgba(157,207,212,0.30)] bg-gradient-to-b from-[rgba(255,255,255,0.94)] to-[rgba(243,249,249,0.88)] px-4 py-3">
              <svg className="h-4 w-4 shrink-0 text-[#6f8c91]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064" />
              </svg>
              <p className="text-xs text-[#6f8c91]">
                Carbon footprint: <span className="font-semibold text-[#10363b]">{insight.weekly_carbon_kg.toFixed(2)} kg CO₂</span> this week
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="mb-4">
          ...
          <div className="rounded-2xl border border-[rgba(157,207,212,0.40)] bg-gradient-to-b from-[rgba(255,255,255,0.94)] to-[rgba(243,249,249,0.88)] p-4 shadow-[0_4px_20px_rgba(0,74,82,0.07)]">
            <div className="mb-2 flex items-center gap-2">
              <div className="flex h-6 w-6 items-center justify-center rounded-full bg-[#E0F4F5]">
        ...
        {hasSchedule && rec?.start_time && (
          <section className="mb-4">
            ...
            <div className="rounded-2xl border border-[#007B8A]/30 bg-[#E0F4F5]/60 p-4">
              <div className="flex items-start gap-3">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#007B8A]/10">
                  <svg className="h-5 w-5 text-[#007B8A]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                </div>
                <div>
                  <p className="text-sm font-semibold text-[#007B8A]">
                  ...
                  <p className="mt-1.5 text-xs text-[#6f8c91]">
          ...
          <section className="mb-4">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 rounded-xl bg-gradient-to-b from-[#86CCD2] to-[#007B8A] px-4 py-3.5 text-sm font-semibold text-white shadow-[0_6px_18px_rgba(0,123,138,0.28)] transition-opacity hover:opacity-90 disabled:opacity-50"
              >
              ...
              <button
                ...
                className="rounded-xl border border-[rgba(157,207,212,0.40)] px-4 py-3.5 text-sm font-medium text-[#4d6b70] transition-colors hover:bg-[#86CCD2]/10 disabled:opacity-50"
              >
                Dismiss
              </button>
            </div>
