...
export function TimeRangeToggle({ value, onChange }: TimeRangeToggleProps): ReactElement {
  return (
    <div className="inline-flex rounded-xl border border-[#86CCD2]/40 bg-white p-1 shadow-sm dark:border-[#86CCD2]/20 dark:bg-zinc-900">
      {options.map((opt) => (
        <button
          key={opt.value}
          type="button"
          ...
          className={`rounded-lg px-4 py-2 text-sm font-medium transition-all duration-200 ${
            value === opt.value
              ...
              : "text-[#666666] hover:bg-[#86CCD2]/10 dark:text-zinc-400 dark:hover:bg-[#86CCD2]/20"
          }`}
        >
          {opt.label}
        </button>
